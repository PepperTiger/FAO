...
ui <- fluidPage(navbarPage("Dashboard"
                           ...
                           ,tabPanel("Population mondial"
                                     , verticalLayout(
                                     
                                     titlePanel("Nombre d'habitants par par pays dans la monde")
                                     
                                     , img(class="img-polaroid",
                                           src=paste0("https://afr100.org/"
                                                      ,"sites/default/files/"
                                                      ,"FAO_logo_Blue_3lines_en.jpg"))
                                     ,mainPanel("Population mondial selon l'année", tableOutput("table"))
                                     ,mainPanel("Top 5  des pays ayant le plus grand nombre de population en moyenne", tableOutput("liste1"))
                ...
                ,tabPanel("Production actuelle"
                          ...
                          ,tagList( selectInput(inputId = "pays3", label = strong("Liste des pays"),
                                                choices = unique(sort(top_hab$country)),
                                                selected = "France")
                                    , selectInput(inputId = "an3", label = strong("Distribution de la production par pays et pa année"),
                ...
                ,tabPanel("Gâchis alimentaire" 
                          ...
                          , selectInput(inputId = "an", label = strong("Distribution de la production par pays et pa année"),
...
server <- function(input, output){
  ...
  #Sous-nutrition
  output$sous_nutri <- renderTable({sous_nutrition %>% head(10)})
  ...
  output$pie <- renderPlot({ggplot(prod_t %>% filter(year == input$an3, country == input$pays3) %>% 
      ...
      labs(titles = "Répartition des items produits", x = "Pays", y = "Production en tonne", fill = "Origine")
  })
  ...
  output$an <- renderPlot({ggplot(prod_t %>% 
      ...
      geom_col() +
      coord_flip() +
      labs(titles = "Distribution de la production par Pays", x = "Pays", y = "Production en tonne", fill = "Origine")
  })
