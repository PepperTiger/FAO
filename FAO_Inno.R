...
fichiers<-list.files(path = "C:/Users/celine/environnement_R_studio/projet_FAO/fichiers_csv", pattern = NULL, all.files = FALSE,
...
for (i in 1:length(fichiers)){
  assign(x=str_sub(fichiers[i],1,-5),
         value=read.csv(paste0("C:/Users/celine/environnement_R_studio/projet_FAO/fichiers_csv/",fichiers[i])))
}



#Concaténation des dataframe bilan_prod_animaux et bilan_vegetaux en ajoutant la colonne "origin"
...
# On vérifie les unités pour les différents champs de la variable Element de temp2 afin de pouvoir transposer notre table
...
#taille de la population mondiale pour chaque année depuis 2014 : 
...
#Création de la table céréales avec la colonne is_cereal
...
#calcul de la proportion de nourriture céréalières destinée aux animaux depuis 2014.
...
###################.
#Disponibilité alimentaire par pays et produit
...
foodmoy<-jointure%>% 
  ...
  group_by(item)%>%
  summarise(moykcal_kg=median(ratio_kcal_kg,na.rm=TRUE),moyprot=100*median(ratio_protein,na.rm=TRUE))%>%
  arrange(desc(moyprot),desc(moykcal_kg))%>%
  head(30)
