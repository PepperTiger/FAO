...
Chemin <- "G:/Mon Drive/AIS_A.I School/Matieres/SQL/FAO/fichiers_csv/"
# Read the folder of csv files
fichiers<-list.files(path = Chemin, pattern = NULL, all.files = FALSE,
...
for (i in 1:length(fichiers)){
  assign(x=str_sub(fichiers[i],1,-5),
         value=read.csv(paste0(Chemin,fichiers[i])))
}



#ConcatÃ©nation des dataframe bilan_prod_animaux et bilan_vegetaux en ajoutant la colonne "origin"
...
# crÃ©ation d'un nouveau dataframe 
colnames(temp)
temp2<-temp%>%select(-c(1,2,5,9,13,14))


# On vÃ©rifie les unitÃ©s pour les diffÃ©rents champs de la variable Element de temp2 afin de pouvoir transposer notre table
...
#taille de la population mondiale pour chaque annÃ©e depuis 2014 : 
population_t%>%filter(country_code!=351)%>%group_by(year)%>%summarise(total=sum(population,na.rm=TRUE))

####CrÃ©ation de la table dispo_alim
...
#CrÃ©ation de la table cÃ©rÃ©ales avec la colonne is_cereal
...
#calcul de la proportion de nourriture cÃ©rÃ©aliÃ¨res destinÃ©e aux animaux depuis 2014.
...
###################â
#DisponibilitÃ© alimentaire par pays et produit
